...
import _default from "bpmn-js/lib/features/append-preview";
...
export default class CustomContextPadProvider extends ContextPadProvider {
    ...
    public getContextPadEntries(element: Element): ContextPadEntries {
        ...
        function appendAction(type: string, className: string, title?: string, options?: object) {
            ...
            const append = (type: string, options: any) => {
                return (event: MouseEvent | TouchEvent) => {
                    const shape = this.elementFactory.createShape(assign({ type }, options));

                    if (this.autoPlace) {
                        this.autoPlace.append(element, shape);
                        appendPreview.cleanUp();
                    } else {
                        console.error('autoPlace is not available');
                    }
                };
            };
